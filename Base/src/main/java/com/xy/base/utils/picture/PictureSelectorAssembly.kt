...
class PictureSelectorAssembly(view:PictureSelectorAssemblyView?) :
    BaseAssemblyWithContext<PictureSelectorAssembly.PictureSelectorAssemblyView>(view){
    ...
    private fun PictureSelectionModel?.requestBase():PictureSelectionModel?{
        ...
        return this?.setLanguage(getLanguage())
            ...
            ?.setRecordAudioInterceptListener(ImageCameraIntercept())
    }
    ...
    interface PictureSelectorAssemblyView : BaseAssemblyViewWithContext {
        fun onCreateSelectType():SelectType = SelectType.ALL
