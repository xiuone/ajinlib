package com.xy.base.widget.tablayout;
...
import com.xy.base.widget.tablayout.listener.OnTabCanClickedListener;
import com.xy.base.widget.tablayout.listener.OnTabSelectListener;
import com.xy.base.widget.tablayout.utils.UnreadMsgUtils;
import com.xy.base.widget.tablayout.widget.MsgView;
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private int mIndicatorMiddleColor;
    private int mIndicatorEndColor;
    //    判断是否是渐变颜色
    private boolean isGradientColor;

//    是否设置Tab的背景色
    private boolean isSetTabBg =false;
//    选中的背景颜色
    private Drawable selectedTabbg;
//    未选中tab的背景颜色
    private Drawable unSelecetedTabBg;

//    第一个和最后一个tab是否设置圆角
    private boolean isSetFirstAndLastTabRadius;
    private Drawable tabFirstSelectRadiusBg;
    private Drawable tabFirstUnSelectRadiusBg;
    private Drawable tabLastSelectRadiusBg;
    private Drawable tabLastUnSelectRadiusBg;
    ...
    //    设置tab是否可以点击跳转(业务需求)
    private int isCanClickPosition=1;
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mTextSelectSize = ta.getDimension(R.styleable.SlidingTabLayout_tl_textSelectSize, mTextUnSelectSize);
        ...
        isGradientColor = ta.getBoolean(R.styleable.SlidingTabLayout_tl_indicator_gradient, false);
        mIndicatorStartColor = ta.getInt(R.styleable.SlidingTabLayout_tl_indicator_start_color, 0);
        mIndicatorMiddleColor = ta.getInt(R.styleable.SlidingTabLayout_tl_indicator_middle_color, 0);
        mIndicatorEndColor = ta.getInt(R.styleable.SlidingTabLayout_tl_indicator_end_color, 0);

        isSetTabBg = ta.getBoolean(R.styleable.SlidingTabLayout_tl_tab_bg, false);
        selectedTabbg = ta.getDrawable(R.styleable.SlidingTabLayout_tl_tab_selected_bg);
        unSelecetedTabBg = ta.getDrawable(R.styleable.SlidingTabLayout_tl_tab_unselected_bg);


        isSetFirstAndLastTabRadius = ta.getBoolean(R.styleable.SlidingTabLayout_tl_tab_radius, false);
        tabFirstSelectRadiusBg = ta.getDrawable(R.styleable.SlidingTabLayout_tl_tab_first_select_radius_bg);
        tabFirstUnSelectRadiusBg = ta.getDrawable(R.styleable.SlidingTabLayout_tl_tab_first_unselect_radius_bg);
        tabLastSelectRadiusBg = ta.getDrawable(R.styleable.SlidingTabLayout_tl_tab_last_select_radius_bg);
        tabLastUnSelectRadiusBg = ta.getDrawable(R.styleable.SlidingTabLayout_tl_tab_last_unselect_radius_bg);
        ta.recycle();
    }
    ...
    private void selectTabClick(int position) {
        if (position != -1) {
            if (mViewPager.getCurrentItem() != position) {
                if (null ==mOnTabCanClickedListener ||  isCanClickPosition == position){
                    if (mSnapOnTabClick) {
                    ...
                    mOnTabCanClickedListener.onTabClicked(position);
                }
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
            ...
            if (isSetTabBg){
                ...
                if (i == mCurrentTab){
                    v.setBackground(selectedTabbg);
                }else {
                    v.setBackground(unSelecetedTabBg);
                }
            }

            if (isSetFirstAndLastTabRadius){
                if (i==0){
                    if (mCurrentTab ==i){
                        v.setBackground(tabFirstSelectRadiusBg);

                    }else {
                        v.setBackground(tabFirstUnSelectRadiusBg);
                    }
                }else  if (i == mTabCount -1 ){
                    if (mCurrentTab ==mTabCount -1){
                        v.setBackground(tabLastSelectRadiusBg);
                    }else {
                        v.setBackground(tabLastUnSelectRadiusBg);
                    }
                }else {
                    if (i == mCurrentTab){
                        v.setBackground(selectedTabbg);
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (isSetTabBg){
                ...
                if (isSelect){
                    tabView.setBackground(selectedTabbg);
                }else {
                    tabView.setBackground(unSelecetedTabBg);
                }
            }
            ...
            if (isSetFirstAndLastTabRadius){
                if (i==0){
                    if (isSelect){
                        tabView.setBackground(tabFirstSelectRadiusBg);

                    }else {
                        tabView.setBackground(tabFirstUnSelectRadiusBg);
                    }
                }else  if (i == mTabCount -1 ){
                    if (isSelect){
                        tabView.setBackground(tabLastSelectRadiusBg);
                    }else {
                        tabView.setBackground(tabLastUnSelectRadiusBg);
                    }
                }else {
                    if (isSelect){
                        tabView.setBackground(selectedTabbg);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIndicatorStyle == STYLE_TRIANGLE) {
            ...
            if (mIndicatorHeight > 0) {
                ...
                if (isGradientColor){
                    if (mIndicatorMiddleColor ==0){ mIndicatorMiddleColor = mIndicatorStartColor;}
                    int[] colors = {mIndicatorStartColor,mIndicatorMiddleColor,mIndicatorEndColor};
                    mIndicatorDrawable.setColors(colors); //添加颜色组
                    mIndicatorDrawable.setGradientType(GradientDrawable.LINEAR_GRADIENT);//设置线性渐变
                    mIndicatorDrawable.setOrientation(GradientDrawable.Orientation.LEFT_RIGHT);//设置渐变方向
                }else {
                    mIndicatorDrawable.setColor(mIndicatorColor);
                }
    ...
    public void setClickedPosition(int position){
        isCanClickPosition = position;
    }


    private OnTabSelectListener mListener;
    private OnTabCanClickedListener mOnTabCanClickedListener;

    public void setOnTabClickedListener(OnTabCanClickedListener mOnTabCanClickedListener) {
        this.mOnTabCanClickedListener = mOnTabCanClickedListener;
    }
