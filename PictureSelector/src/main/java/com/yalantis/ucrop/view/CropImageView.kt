...
open class CropImageView @JvmOverloads constructor(
    ...
    fun cropAndSaveImage(
        compressFormat: Bitmap.CompressFormat, compressQuality: Int,
        cropCallback: BitmapCropCallback?
    ) {
        cancelAllAnimations()
        setImageToWrapCropBounds(false)
        val imageState = ImageState(
            mCropRect, trapToRect(mCurrentImageCorners),
        ...
        cropParameters.contentImageInputUri = imageInputUri
        cropParameters.contentImageOutputUri = imageOutputUri
    ...
    private fun calculateImageIndents(): FloatArray {
        ...
        indents[0] = if (deltaLeft > 0) deltaLeft else 0
        indents[1] = if (deltaTop > 0) deltaTop else 0
        indents[2] = if (deltaRight < 0) deltaRight else 0
        indents[3] = if (deltaBottom < 0) deltaBottom else 0
        mTempMatrix.reset()
        mTempMatrix.setRotate(currentAngle)
    ...
    fun processStyledAttributes(a: TypedArray) {
        val targetAspectRatioX = Math.abs(
            a.getFloat(
                R.styleable.ucrop_UCropView_ucrop_aspect_ratio_x,
                DEFAULT_ASPECT_RATIO
            )
        )
        val targetAspectRatioY = Math.abs(
            a.getFloat(
    ...
    private class WrapCropBoundsRunnable(
        ...
        override fun run() {
            ...
            val newX = easeOut(currentMs, 0f, mCenterDiffX, mDurationMs.toFloat())
            val newY = easeOut(currentMs, 0f, mCenterDiffY, mDurationMs.toFloat())
            val newScale = easeInOut(currentMs, 0f, mDeltaScale, mDurationMs.toFloat())
